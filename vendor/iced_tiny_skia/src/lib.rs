            ...
            for layer in self.layers.iter() {
                ...
                if !layer.primitives.is_empty() {
                    ...
                    for group in &layer.primitives {
                        // PATCH glacier-ui — a mesma correção que já está no
                        // master do iced. O recorte já chega transformado de
                        // `draw_primitive_group` (ele guarda
                        // `clip_bounds * transformation`), e multiplicar por
                        // `group.transformation()` outra vez aplicava a
                        // translação DUAS vezes: numa 0.14.0 publicada, o
                        // primeiro canvas da tela saía cortado e todos os
                        // seguintes sumiam. Some quando a 0.14.1 sair.
